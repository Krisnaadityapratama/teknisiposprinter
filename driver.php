...
<body class="bg-gray-50 font-sans antialiased">
    ...
    <main class="max-w-7xl mx-auto mt-12 px-6">
        ...
        <!-- Search Bar -->
        <div class="mb-6">
            <input type="text" id="search" placeholder="Cari driver..." class="w-full px-4 py-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-300">
        </div>

        <!-- Menampilkan total jumlah driver -->
        <div class="mb-6 text-lg text-gray-800">
            <p>Total Driver: <span class="font-semibold"><?= $totalDrivers ?></span></p>
        </div>

        <!-- File List -->
        <div id="file-list" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php foreach ($driverFiles as $file): ?>
                <div class="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition duration-300 transform hover:-translate-y-1">
                    ...
                    <a href="<?= $driversFolder . '/' . urlencode($file) ?>" download class="inline-block bg-blue-500 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition duration-300">Download</a>
                </div>
            <?php endforeach; ?>
        </div>
    </main>
